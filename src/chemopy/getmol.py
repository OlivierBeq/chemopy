...
class MolFrom:
        ...
        for i in temp:
            if re.findall("InChI=", i) == ["InChI="]:
                ...
                if kk[0][0:5] == "InChI":
                    ...
                    res = "None"
        if res is None:
            raise ValueError(f"Could not find an InChI for CAS ID {casid!r} in the ChemNet response.")
    ...
    def EBI(chebi_id: str = "") -> Chem.Mol:
        ...
        if chid.startswith("CHEBI"):
            ...
            request = urllib.request.Request(
                f"https://www.ebi.ac.uk/chembl/api/data/molecule?chembl_id={chid}&format=sdf"
            )
            with urllib.request.urlopen(request) as response:
    ...
    def NCBI(cid: str = "") -> Chem.Mol:
        ...
        request = urllib.request.Request(
            f"http://pubchem.ncbi.nlm.nih.gov/summary/summary.cgi?cid={cid}&disopt=SaveSDF"
        )
        with urllib.request.urlopen(request) as response:
    ...
    def DrugBank(drugbank_id: str = "") -> Chem.Mol:
        ...
        request = urllib.request.Request(
            f"https://go.drugbank.com/structures/small_molecule_drugs/{dbid}.sdf",
            headers={
